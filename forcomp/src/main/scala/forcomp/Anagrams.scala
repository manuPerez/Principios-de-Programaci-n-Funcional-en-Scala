...
object Anagrams{
   ...
   */
  def wordOccurrences(w: Word): Occurrences = w.toLowerCase.groupBy(c => c).map(c => (c._1, c._2.length)).toList.sorted
  ...
  lazy val dictionaryByOccurrences: Map[Occurrences, List[Word]] =
    dictionary.map(c => (wordOccurrences(c), c)).groupBy(c => c._1).map(c => (c._1, c._2.map(a => a._2)))
